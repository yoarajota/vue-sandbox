<template>
  <VCard ref="resizableCard" width="250px" class="duration-10 overflow-auto"
    :style="{ top: `${state.y}px`, left: `${state.x}px`, height: `${state.height}px`, width: `${state.width}px` }"
    position="absolute">
...
function presetPositionMenu(position = 0) {
  if (position === 0) {
    state.x = 60;
  } else {
    state.x = window.innerWidth - 250 - 60;
  }
}
